...
//!!!Important, import the Krycklan ... from data subfolder and rename it to geometry

/*function snow_mask(img){
  var not_snow = img.select('SCL').neq(11).rename('not_snow');
  return img.addBands(not_snow);
}

var s2_sr_col = ee.ImageCollection('COPERNICUS/S2_SR').filterDate('2018-02-01', '2018-02-15').filterBounds(geometry)

var s2_dataset = s2_sr_col.map(snow_mask)

print(s2_dataset)*/



var START = '2018-02-01'
var END = '2018-02-15'
...
//Function for masking snow
/*function snow_free(img){
  var not_snow = img.select('SCL').neq(11);
  return img.updateMask(not_snow)
}*/

function snow_mask(img){
  var snow = img.select('SCL').eq(11).multiply(10000).rename('snow');
...
function apply_cld_shdw_mask(img) {
    ...
    return img.select('B.*', 'snow').updateMask(not_cld_shdw)
}

var s2_sr_cld_col = get_s2_sr_cld_col(AOI, START_DATE, END_DATE)
/*var s2_dataset = s2_sr_cld_col.map(snow_free)*/
var s2_dataset = s2_sr_cld_col.map(snow_mask)
...
batch.Download.ImageCollection.toDrive(collection_MOD_albedo, 'ANNA_MODIS_ALBE_ALL', 
  {name: '{system:id}',
  scale: 500,
  maxPixels: 3784216672400,
  region: geometry,
